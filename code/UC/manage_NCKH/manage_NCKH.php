...
$columns = [
    "id", "employeeID", "result_year", "noi_dung", "ten_san_pham", "so_luong", "vai_tro", "so_tac_gia", "phan_tram_dong_gop",
    "gio_quy_doi", "ngay_cap_nhat", "diem_tap_chi", "ma_so_xuat_ban", "ten_don_vi_xuat_ban", "ten_hoi_thao", "thanh_vien_chu_nhom", "nation_point", "student_infor"
];
$labels = [
    "STT", "Mã giảng viên", "Năm kết quả", "Nội dung", "Tên sản phẩm", "Số lượng", "Vai trò", "Số tác giả", "Phần trăm đóng góp",
    "Giờ quy đổi", "Ngày cập nhật", "Điểm tạp chí", "Mã số xuất bản", "Tên đơn vị xuất bản", "Tên hội thảo", "Tên chủ nhóm", "Thể loại", "Đại diện sinh viên"
];
?>
<!DOCTYPE html>
<html lang="vi">
<head>
    ...
    <style>
        ...
        #nckhTable th:nth-child(4), #nckhTable td:nth-child(4) {
            min-width: 300px;
            max-width: 300px;
            white-space: normal;
            word-wrap: break-word;
...
<div class="container py-4">
    ...
    <!-- Ô tìm kiếm -->
    <div class="box-section">
        <input type="text" id="globalSearch" class="form-control" placeholder="Tìm kiếm theo tên sản phẩm...">
    </div>
    <!-- Bảng dữ liệu -->
        ...
        <table id="nckhTable" class="table table-striped table-hover align-middle">
            ...
            <tbody>
                ...
                try {
                    ...
                    if ($results) {
                        // Nhóm dữ liệu theo ten_san_pham, thanh_vien_chu_nhom và result_year
                        $grouped_data = [];
                        foreach ($results as $row) {
                        ...
                        foreach ($grouped_data as $group) {
                            ...
                            foreach ($group as $index => $row) {
                                ...
                                echo "<td data-column='3'>" . htmlspecialchars($row['noi_dung']) . "</td>";
                                echo "<td data-column='4'>" . htmlspecialchars($row['ten_san_pham']) . "</td>";
                                echo "<td data-column='5'>" . htmlspecialchars($row['so_luong']) . "</td>";
                                echo "<td data-column='6'>" . htmlspecialchars($row['vai_tro']) . "</td>";
                                echo "<td data-column='7'>" . htmlspecialchars($row['so_tac_gia']) . "</td>";
                                echo "<td data-column='8'>" . htmlspecialchars($row['phan_tram_dong_gop']) . "</td>";
                                echo "<td data-column='9'>" . htmlspecialchars($row['gio_quy_doi']) . "</td>";
                                echo "<td data-column='10'>" . htmlspecialchars($row['ngay_cap_nhat']) . "</td>";
                                echo "<td data-column='11'>" . htmlspecialchars($row['diem_tap_chi']) . "</td>";
                                echo "<td data-column='12'>" . htmlspecialchars($row['ma_so_xuat_ban']) . "</td>";
                                echo "<td data-column='13'>" . htmlspecialchars($row['ten_don_vi_xuat_ban']) . "</td>";
                                echo "<td data-column='14'>" . htmlspecialchars($row['ten_hoi_thao']) . "</td>";
                                echo "<td data-column='15'>" . htmlspecialchars($row['thanh_vien_chu_nhom']) . "</td>";
                                echo "<td data-column='16'>" . htmlspecialchars($row['nation_point']) . "</td>";
                                echo "<td data-column='17'>" . htmlspecialchars($row['student_infor']) . "</td>";
                                echo "</tr>";
                            }
                        }
                    }
    ...
    <div class="d-flex justify-content-between mt-4">
        <button class="btn btn-success" onclick="exportTable()"><i class="fas fa-file-export"></i> Xuất báo cáo</button>
...
<script>
// Define exportTable function in global scope
function exportTable() {
    // Chuyển hướng đến file export
    window.location.href = 'export_table.php';
}
...
$(document).ready(function() {
    ...
    var columnMap = {
        ...
        'diem_tap_chi': 11,
        'ma_so_xuat_ban': 12,
        'ten_don_vi_xuat_ban': 13,
        'ten_hoi_thao': 14,
        'nation_point': 16
    };

    // Ensure "Nội dung" column (index 3) and "Mã số xuất bản" (column 12) are checked by default
    $('.toggle-col[data-column="3"]').prop('checked', true).trigger('change');
    $('.toggle-col[data-column="12"]').prop('checked', true).trigger('change');
    ...
    function applyFilters() {
        ...
        $('#nckhTable tbody tr').each(function() {
            ...
            // Check global search - only search in "Tên sản phẩm" column (index 4)
            if (showRow && searchText) {
                var productNameCell = row.find('td[data-column="4"]');
                var productName = productNameCell.text().toLowerCase();
