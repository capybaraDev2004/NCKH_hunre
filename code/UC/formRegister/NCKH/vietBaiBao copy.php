...
const GIANG_VIEN_LIST = <?php
    ...
    $stmtGV = $conn->prepare("SELECT employeeID, teacherID, fullName FROM employee");
    $stmtGV->execute();
    ...
    button.addEventListener('click', function(e) {
        ...
        formRow.querySelector('#saveBaiBaoForm').addEventListener('submit', function(e) {
            ...
            for (let i = 0; i < soTacGia; i++) {
                ...
                // Copy các trường chung
                singleFormData.append('type', formData.get('type'));
                singleFormData.append('id', baiBaoId);
                singleFormData.append('noi_dung', formData.get('noi_dung'));
                singleFormData.append('selected_year_form', formData.get('selected_year_form'));
                singleFormData.append('ten_san_pham', formData.get('ten_san_pham'));
                singleFormData.append('so_luong', formData.get('so_luong'));
                singleFormData.append('diem_tap_chi', formData.get('diem_tap_chi'));
                singleFormData.append('ma_so_xuat_ban', formData.get('ma_so_xuat_ban'));
                singleFormData.append('ten_don_vi_xuat_ban', formData.get('ten_don_vi_xuat_ban'));
                singleFormData.append('ten_hoi_thao', formData.get('ten_hoi_thao'));
                singleFormData.append('so_tac_gia', soTacGia);

                // Trường riêng từng tác giả
                singleFormData.append('thanh_vien[]', thanhVienInputs[i].value);
                singleFormData.append('phan_tram_dong_gop[]', phanTramInputs[i].value);
                singleFormData.append('role_thanh_vien[]', roleInputs[i].value);

                // Tính note cho từng tác giả
                ...
                singleFormData.append('note[]', noteVal);

                fetch('/demo_web/Nghiệp vụ giảng viên/code/UC/formRegister/NCKH/add.php', {
                    method: 'POST',
                    body: singleFormData
                })
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        failCount++;
                        alert('Lưu thất bại cho tác giả: ' + (data.message || 'Vui lòng thử lại.'));
                    }
...
function handleDelete() {
    ...
    if (confirm('Bạn có chắc chắn muốn xóa mục này không?')) {
        ...
        fetch('/demo_web/Nghiệp vụ giảng viên/code/UC/formRegister/NCKH/delete.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
