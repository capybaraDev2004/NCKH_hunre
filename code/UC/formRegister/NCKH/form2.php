...
$statsYear = isset($_POST['stats_year']) ? $_POST['stats_year'] : (date('Y') - 1);
...
try {
    ...
    $totalResearchHours += $stmt->fetchColumn();
...
// Lấy note dựa trên năm được chọn trong form
...
if ($noteValue === null || $noteValue === '') {
    // Nếu không có note, sử dụng giá trị mặc định
    $standardTeachingHours = 330; // Giảng dạy trực tiếp mặc định = 330
    $standardResearchHours = 590; // NCKH mặc định = 590
} else {
switch ($noteValue) {
    case 1:
        $standardTeachingHours = 270; // Giảng dạy trực tiếp = 270
        $standardResearchHours = 770; // NCKH = 770
        break;
    case 2:
        $standardTeachingHours = 300; // Giảng dạy trực tiếp = 300
        $standardResearchHours = 680; // NCKH = 680
        break;
    default:
        $standardTeachingHours = 330; // Giảng dạy trực tiếp = 330
        $standardResearchHours = 590; // NCKH = 590
        break;
    }
}

$standardOtherTasksHours = 180; // Giá trị mặc định cho nhiệm vụ khác
...
function calculateTargetTeachingHours($academicTitle, $leadershipPosition, $conn, $employeeID) {
    ...
    switch ($academicTitle) {
        ...
        case 'Giảng viên (tập sự)':
            $baseHours = $standardTeachingHours * 0.5;
            break;
        case 'Trợ giảng':
            $baseHours = $standardTeachingHours * 0.5;
            break;
        case 'Trợ giảng (tập sự)':
            $baseHours = $standardTeachingHours * 0.424;
            break;
        default:
            $baseHours = $standardTeachingHours;
    }
    ...
    if ($academicTitle === 'Giảng viên') {
        $leadershipPercentages = [
            'Chủ tịch Hội đồng trường, Hiệu trưởng' => 0.15,
            'Phó chủ tịch Hội đồng trường, Phó hiệu trưởng' => 0.20,
            'Trưởng phòng, Giám đốc trung tâm, Thư ký Hội đồng trường' => 0.25,
            'Phó trưởng phòng, Phó Giám đốc trung tâm' => 0.30,
            'Trưởng khoa, Trưởng Bộ môn trực thuộc trường (≥ 40 giảng viên hoặc ≥ 800 người học)' => 0.60,
            'Trưởng khoa, Trưởng Bộ môn trực thuộc trường (< 40 giảng viên hoặc < 800 người học)' => 0.70,
            'Phó trưởng khoa, Phó trưởng bộ môn trực thuộc trường (≥ 40 giảng viên hoặc ≥ 800 SV)' => 0.70,
            'Phó trưởng khoa, Phó trưởng bộ môn trực thuộc trường (< 40 giảng viên hoặc < 800 SV)' => 0.80,
            'Trưởng bộ môn trực thuộc khoa' => 0.80,
            'Phó trưởng bộ môn trực thuộc khoa' => 0.85,
            'Chủ nhiệm lớp, Cố vấn học tập (được cộng dồn)' => 0.85,
            'Bí thư đảng ủy' => 0.15,
            'Phó bí thư Đảng ủy' => 0.30,
            'Bí thư chi bộ, Trưởng ban TTND, Trưởng Ban nữ công, Chủ tịch Hội CCB' => 0.85,
            'Phó Bí thư chi bộ' => 0.90,
            'Giảng viên làm công tác quốc phòng, quân sự không chuyên trách' => 0.80,
            'Bí thư đoàn trường' => 0.30,
            'Phó bí thư đoàn trường' => 0.40,
            'Bí thư Liên chi đoàn (≥ 1,000 SV)' => 0.60,
            'Bí thư Liên chi đoàn (500 - 1,000 SV)' => 0.65,
            'Bí thư Liên chi đoàn (< 500 SV)' => 0.70,
            'Giảng viên nữ có con nhỏ dưới 12 tháng' => 0.90,
            'Cán bộ giảng dạy kiêm quản lý phòng thí nghiệm, thực hành công nghệ, phòng máy' => 0.85,
            'Chủ tịch Công đoàn trường, Phó chủ tịch Công đoàn trường' => 0.8667,
            'Ủy viên BCH công đoàn trường, tổ trưởng, tổ phó tổ công đoàn' => 0.9333,
            'Giảng viên làm thay công tác trợ lý khoa nghỉ việc' => 0.50,
            'Giảng viên đang là quân nhân dự bị, tự vệ được triệu tập huấn luyện, diễn tập' => 1.00
        ];
        ...
        if (array_key_exists($leadershipPosition, $leadershipPercentages)) {
            if ($leadershipPosition === 'Giảng viên đang là quân nhân dự bị, tự vệ được triệu tập huấn luyện, diễn tập') {
                ...
                if ($trainingData) {
                    ...
                    $targetHours = $standardTeachingHours - ($days * 2.5);
                } else {
                    $targetHours = $standardTeachingHours;
                }
            } else {
...
// Hàm tính số giờ nhiệm vụ khác dựa trên chức danh
...
if (isset($_POST['action'])) {
    if ($_POST['action'] === 'get_stats') {
        ...
        // Thiết lập giờ chuẩn dựa trên note
        if ($noteValue === null || $noteValue === '') {
            // Nếu không có note, sử dụng giá trị mặc định
            $standardTeachingHours = 330; // Giảng dạy trực tiếp mặc định = 330
            $standardResearchHours = 590; // NCKH mặc định = 590
        ...
        // Tính toán lại targetTeachingHours dựa trên standardTeachingHours mới
        $targetTeachingHours = calculateTargetTeachingHours($employee['academicTitle'], $employee['leadershipPosition'], $conn, $employeeID);

        $standardOtherTasksHours = 180;
        ...
        // Tính giờ quy đổi từ số ngày diễn tập
        $stmt = $conn->prepare("SELECT so_ngay FROM dien_tap WHERE employeeID = :employeeID AND result_year = :result_year");
        $stmt->execute([':employeeID' => $employeeID, ':result_year' => $year]);
        ...
        // Get research hours
        $researchTables = ['nckhcc_history', 'bai_bao_history', 'huongdansv_history', 'vietsach_history'];
        foreach ($researchTables as $table) {
            $stmt = $conn->prepare("SELECT COALESCE(SUM(gio_quy_doi), 0) as total FROM $table WHERE employeeID = :employeeID AND result_year = :result_year");
            $stmt->execute([':employeeID' => $employeeID, ':result_year' => $year]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $currentPage === 'teaching' && isset($_POST['selected_year']) && !isset($_POST['action'])) {
    ...
    if ($selectedYear != (date('Y') - 1)) {
        ...
        if ($exists) {
            echo "<script>
                showConfirmPopup('Dữ liệu cho năm $selectedYear đã tồn tại. Bạn có chắc chắn muốn ghi đè không?', function() {
                    fetch('', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                        body: '" . http_build_query($_POST) . "&confirm_overwrite=1'
                    }).then(() => {
...
$tongGioChuan = 0;
$tongGioNCKH = $totalResearchHours;
$tongGioNhiemVuKhac = 0;
...
// Tự động lưu giá trị mặc định cho bảng giangday
$currentYear = date('Y');
...
<body>  
    <div class="user-header">
        ...
        <div class="user-profile">
            ...
            <ul class="subnav">
                ...
                <li><a href="./NCKH/test.php"><i class="fas fa-sign-out-alt"></i>test</a></li>
    ...
    <div class="container">
                ...
                <li class="sidebar-item">
                    ...
                    <ul class="submenu <?php echo $currentPage === 'other_tasks' ? 'submenu-active' : ''; ?>" id="other-tasks-submenu">
                        ...
                        <li class="submenu-item">
                            <a href="formRegister.php?page=other_tasks&subpage=nv_khac_b&section=b1" class="sidebar-link <?php echo $subpage === 'nv_khac_b' ? 'active' : ''; ?>">
            ...
            <div style="width: 100%; text-align: center;">
                <select id="statsYear" name="stats_year" onchange="updateStats(this.value)" style="padding: 8px; font-size: 14px; border-radius: 4px; background-color: #fff; color: #223771; width: 80%;">
                    <?php
                        ...
                        echo "<option value='$i'" . ($i == date('Y') - 1 ? " selected" : "") . ">$i</option>";
                        }
                    ?>
                </select>
            </div>
        ...
        <div class="target-hours" id="teaching-status"></div> <!-- Loại bỏ PHP logic, để trống để JavaScript cập nhật -->
        <canvas id="teachingChart"></canvas>
    </div>
    <div class="chart-container">
        <div class="chart-title">Thống kê NCKH</div>
        <div class="target-hours" id="research-status"></div> <!-- Loại bỏ PHP logic, để trống để JavaScript cập nhật -->
        <canvas id="researchChart"></canvas>
    </div>
    <div class="chart-container">
        <div class="chart-title">Thống kê NV Khác</div>
        <div class="target-hours" id="other-tasks-status"></div> <!-- Loại bỏ PHP logic, để trống để JavaScript cập nhật -->
