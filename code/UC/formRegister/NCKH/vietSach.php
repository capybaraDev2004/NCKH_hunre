...
// Lấy năm từ dropdown hoặc form động, mặc định là năm hiện tại
...
// Lấy danh sách viết sách
$stmt = $conn->prepare("SELECT ID, NoiDung, KhoiLuongGio FROM vietsach");
$stmt->execute();
...
$filteredSachList = $sachList;
if ($searchQuery) {
    $filteredSachList = array_filter($sachList, function($item) use ($searchQuery) {
        return stripos($item['NoiDung'], $searchQuery) !== false;
    });
}
?>

<h2 class="form-title">Viết sách</h2>
...
<div class="search-container">
    <label for="search">Tìm kiếm: </label>
        <input type="text" id="search" name="search" value="<?php echo htmlspecialchars($searchQuery); ?>" placeholder="Nhập nội dung sách..." onkeyup="searchTable()" class="search-input">
    </div>
</div>

<!-- Bảng danh sách sách -->
<form method="POST" id="sachForm">
    <table style="width: 100%; border-collapse: collapse; margin-top: 20px;" id="sachTable">
        <thead>
            <tr style="background-color: #223771; color: white;">
                <th style="padding: 10px; width: 5%;">STT</th>
                <th style="padding: 10px; width: 70%;">Nội dung sách</th>
                <th style="padding: 10px; width: 15%;">Khối lượng giờ</th>
                <th style="padding: 10px; width: 10%;">Đăng ký</th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($filteredSachList as $sach): ?>
                <tr data-sach-id="<?php echo $sach['ID']; ?>">
                    <td style="padding: 10px; text-align: center;"><?php echo htmlspecialchars($sach['ID']); ?></td>
                    <td style="padding: 10px; text-align: left;"><?php echo htmlspecialchars($sach['NoiDung']); ?></td>
                    <td style="padding: 10px; text-align: center;" class="khoi-luong-gio" data-sach-id="<?php echo $sach['ID']; ?>">
                        <?php echo htmlspecialchars($sach['KhoiLuongGio']); ?>
                    </td>
                    <td style="padding: 10px; text-align: center;">
                        <button type="button" name="selected_sach_id" value="<?php echo $sach['ID']; ?>" class="register-btn">Đăng ký</button>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
<table style="width: 100%; border-collapse: collapse; margin-top: 10px;" id="historyTable">
    <thead>
        <tr style="background-color: #223771; color: white;">
            <th style="padding: 10px; width: 5%;">STT</th>
            <th style="padding: 10px; width: 20%;">Nội dung sách</th>
            <th style="padding: 10px; width: 20%;">Tên sách</th>
    ...
    <tbody>
        ...
        $historyStmt = $conn->prepare("
            ...
            FROM vietsach_history vh
            LEFT JOIN vietsach v ON vh.nckh_id = v.ID
        ...
        if ($historyData) {
            ...
            foreach ($historyData as $row) {
                ...
                echo "<tr data-sach-id='{$row['nckh_id']}' data-id='{$row['id']}'>
...
document.querySelectorAll('.register-btn').forEach(button => {
    button.addEventListener('click', function(e) {
        ...
        formRow.innerHTML = `
            <td colspan="4">
                <h3>Đăng ký viết sách: ${noiDung} (Năm ${selectedYear})</h3>
                <form id="saveSachForm" class="styled-form">
                    <input type="hidden" name="type" value="vietsach">
                    <input type="hidden" name="id" value="${sachId}">
        ...
        // Xử lý lưu bằng AJAX
        formRow.querySelector('#saveSachForm').addEventListener('submit', function(e) {
            e.preventDefault();
...
function handleDelete() {
    ...
    if (confirm('Bạn có chắc chắn muốn xóa mục này không?')) {
        ...
        formData.append('action', 'delete_sach');
